...
public class PlayerAI : MonoBehaviour {
    ...
    void CheckIfInHighscores () {
        ...
        for (int i = 0; i < 8; i++) {
            if (upgradePoints > currentHighscores[i]) {
                ...
                GameObject.FindGameObjectWithTag("Systems Process").GetComponent<SystemsProcess>().saveHighScore = true;
                GameObject.FindGameObjectWithTag("Systems Process").GetComponent<SystemsProcess>().highscorepalce = i;
                break;
            }
        }
    }
    ...
    public void Health(int value) {
        if (isSpecialAttackUnderWay == false) {
            if (isControlOn == true) {
                ...
                GetComponent<PlaySoundEffect>().PlaySound(7);
            }
            if (currenthealth <= 0) {
                isDead = true;
                //AnimSwitchTo("isDead");
                GameObject.FindGameObjectWithTag("Systems Process").GetComponent<SystemsProcess>().OpenPrompt(9);
    ...
    public void BuyWeapon(int type) {
        ...
        //BRUH
        if (type == 420) {
            GetComponent<PlaySoundEffect>().PlaySound(14);
        }
        else {
            GetComponent<PlaySoundEffect>().PlaySound(12);
        }
        ...
        if (type == 2) {
            ...
            if (systemsProcSpawn.level == 29 && upgradePoints >= 70000) {
                ...
                buybutton.SetActive(false);
                buybutton.GetComponent<Button>().onClick.RemoveAllListeners();
                GameObject.FindGameObjectWithTag("Systems Process").GetComponent<SystemsProcess>().hud.SetActive(false);
                GameObject.FindGameObjectWithTag("Systems Process").GetComponent<SystemsProcess>().gamePause.SetActive(false);
                //Health(-100);
    ...
    public void attackDamageTickedSet() {
        ...
        if (attackMode == 2 || attackMode == 3 || attackMode == 4 || attackMode == 5) {
            CylinderTargetArea targetarea = cylinderTargetArea.GetComponent<CylinderTargetArea>();
            for (int i = 0; i < listOfTargets.Count; i++) {
                if (targetarea.withinArea.Contains(listOfTargets[i]) == true && Vector3.Distance(transform.position, listOfTargets[i].transform.position) <= attackRange) {
